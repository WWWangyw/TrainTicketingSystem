...
public class DeleteUserServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(id==null || "".equals(id.trim())){
			out.println("<script>alert('要删除的数据不存在！');window.location.href='/TrainStation/GetUsersServlet'</script>");
			out.close();
			return;
		}
		//调用userService层的方法
		...
		if(result>0){
			out.println("<script>alert('删除成功！');window.location.href='/TrainStation/GetUsersServlet'</script>");
			out.close();
		}else{
