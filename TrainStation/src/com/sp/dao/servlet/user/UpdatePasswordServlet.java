...
public class UpdatePasswordServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(id==null || "".equals(id.trim())){
			...
			out.println("<script>alert('请重新登陆！');window.location.href='/TrainStation/login.jsp'</script>");
			out.close();
			return;
		}
		...
		if( !user.getPassword().equals(oldPassword) ){
			...
			if(result>0){
				out.println("<script>alert('密码修改成功！');window.location.href='/TrainStation/files/personalInformation.jsp'</script>");
				out.close();
			}else{
