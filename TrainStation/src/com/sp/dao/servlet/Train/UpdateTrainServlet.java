...
public class UpdateTrainServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(list.size()>0){
			out.println("<script>alert('要修改的车次已有订单，不允许修改！');window.location.href='/TrainStation/GetTrainsServlet'</script>");
			out.close();
			return;
		}
		...
		if(result>0){
			out.println("<script>alert('修改成功!');window.location.href='/TrainStation/GetTrainsServlet';</script>");
			out.close();
		}else{
			out.println("<script>alert('修改失败，请重试！');window.location.href='/TrainStation/GetTrainsServlet'</script>");
			out.close();
		}
	}
