...
public class DeleteTrainServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(train_num==null || "".equals(train_num.trim())){
			out.println("<script>alert('要删除的数据不存在！');window.location.href='/TrainStation/GetTrainsServlet'</script>");
			out.close();
			return;
		}
		...
		if(result>0){
			out.println("<script>alert('删除成功！');window.location.href='/TrainStation/GetTrainsServlet'</script>");
			out.close();
		}else{
